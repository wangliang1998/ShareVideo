      ...
      <el-header style="background:black" height="70">
        <div  style="width: 100%;">
          <div class="logo margin-big-left fadein-top" style="width: 100%;text-align: left">
            <el-avatar :size="50" :src="user_icon" style="text-align: left;margin-top:10px;float: left"></el-avatar>
            <h1 style="margin-left: 10px;color: white;float: left">{{username}}的个人管理</h1>
          </div>
        </div>
      </el-header>
      <el-container>
            ...
            <el-col :span="12" style="width: 200px">
              <el-menu
                ...
                <el-submenu index="2">
                  ...
                  <el-menu-item-group>
                    <el-menu-item index="2-1" @click="to(2)"><i class="el-icon-video-camera"></i>个人视频中心</el-menu-item>
...
export default {
  ...
  methods: {
    ...
    to (index) {
      if (index === 1) {
        this.url = '/user/back/user'
      } else if (index === 2) {
        this.url = '/center/?userid=' + localStorage.getItem('username')
      } else if (index === 3) {
        this.url = '/user/back/video/update'
      } else if (index === 4) {
        this.url = '/user/back/comment/update'
      }
    }
  },
  created () {
    document.title = localStorage.getItem("username") + '  管理系统'
    this.user_icon = axios.defaults.baseURL + localStorage.getItem("icon")
